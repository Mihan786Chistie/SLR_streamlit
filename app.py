...
class SLRParser:
                ...
                for body in bodies.copy():
                    if '.' in body[:-1]:
                        ...
                        if symbol_after_dot in self.G_prime.nonterminals:
                            for G_body in self.G_prime.grammar[
                                    symbol_after_dot]:
                                J.setdefault(symbol_after_dot,
                                             set()).add(('.', ) if G_body == (
                                                 '^', ) else ('.', ) + G_body)
                ...
                for body in bodies:
                    if '.' in body[:-1]:  # CASE 2 a
                        ...
                        if symbol_after_dot in self.G_prime.terminals:
                            ...
                            -1] == '.' and head != self.G_prime.start:  # CASE 2 b
                        for j, (G_head, G_body) in enumerate(self.G_indexed):
                            if G_head == head and (G_body == body[:-1]
                                                   or G_body == ('^', )
                                                   and body == ('.', )):
    ...
    def print_info(self):
        ...
        def print_line():
            return (
                f'+{("-" * width + "+") * (len(list(self.G_prime.symbols) + ["$"]))}'
            )

        def symbols_width(symbols):
            return (width + 1) * len(symbols) - 1

        output = []  # Store output lines
        output.append('AUGMENTED GRAMMAR:')

        for i, (head, body) in enumerate(self.G_indexed):
            output.append(
                f'{i:>{len(str(len(self.G_indexed) - 1))}}: {head:>{self.max_G_prime_len}} -> {" ".join(body)}'
            )

        output.append('')
        output.append(fprint('TERMINALS', self.G_prime.terminals))
        output.append(fprint('NONTERMINALS', self.G_prime.nonterminals))
        output.append(fprint('SYMBOLS', self.G_prime.symbols))

        output.append('\nFIRST:')
        for head in self.G_prime.grammar:
            output.append(
                f'{head:>{self.max_G_prime_len}} = {{ {", ".join(self.first[head])} }}'
            )

        output.append('\nFOLLOW:')
        for head in self.G_prime.grammar:
            output.append(
                f'{head:>{self.max_G_prime_len}} = {{ {", ".join(self.follow[head])} }}'
            )
        ...
        for r in range(len(self.C)):
            for c in self.parse_table_symbols:
                if isinstance(self.parse_table[r][c], int):
                    automaton.edge(f'I{r}',
                                   f'I{self.parse_table[r][c]}',
                                   label=f'<<I>{c}</I>>')

                elif 's' in self.parse_table[r][c]:
                    i = self.parse_table[r][c][self.parse_table[r][c].
                                               index('s') + 1:]
                    ...
                    automaton.edge(f'I{r}',
                                   f'I{i}',
                                   label=f'<<B>{c}</B>>'
                                   if c in self.G_prime.terminals else c)
    ...
    def print_LR_parser(self, results):
        ...
        max_lens = {
            key: max(len(value) for value in results[key])
            for key in results
        }
        justs = {
            'step': '>',
...
def main():
    ...
    if uploaded_file is not None:
        ...
        # Display grammar information using print_info()
        st.header("Grammar Information")
        ...
        if generate_automaton:
            ...
            st.header("Parsing Automaton")
            automaton = slr_parser.generate_automaton()
            st.graphviz_chart(automaton)
